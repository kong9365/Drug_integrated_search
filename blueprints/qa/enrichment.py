...
def enrich_one(item: Dict) -> Dict:
    ...
    # 5) 한약(생약) 분기 — is_herbal 품목은 NO 35 허가기원(생약명)으로 폴백 (NO140/145 실패 시)
    #    ※ NO35는 생약명↔동식물명 기원 정보(허가번호·ITEM_SEQ 없음) → confidence 'low'
    if item.get("is_herbal"):
        try:
            from ...api_extras import fetch_herbal_approval
            h = fetch_herbal_approval(drnm=base, num_of_rows=5)
            rows = h.get("items", []) if h.get("success") else []
            if rows:
                r0 = rows[0]
                result["material_name"] = r0.get("DRNM") or r0.get("HBMD_NM") or base
                result["source_api"] = "NO35"
                result["enrich_status"] = "matched"
                result["enrich_confidence"] = "low"
                return result
        except Exception as e:
            logger.debug(f"[{item_code}] NO35 한약 폴백 실패: {e}")
