...
> **문서 버전**: v1.2 (2026-05-26 · 신규 5개 승인 반영, 마스크 제외)
> **현황**: ✅ **18개 API smoke test 통과** · 🟡 5개 승인됐으나 End Point 확인 대기 · ⚪ 마스크 2개 프로젝트 제외
> **관련 문서**: [API_APPLICATION_LIST.md](API_APPLICATION_LIST.md) · [API_STATUS.md](API_STATUS.md) · [smoke_test_apis.py](smoke_test_apis.py) · [README.md](README.md)
...
### 1-3. 기존 자산 (2026-05-26 현황)
| 자산 | 위치 | 상태 |
|---|---|---|
| Flask 웹앱 | [app.py](app.py) | 4개 API 연동 (허가/처분/회수/낱알) |
| API 클라이언트 | [api_client.py](api_client.py) | 재시도·SSL 폴백 로직 + NO 140 함수명 수정 완료 |
| 엑셀 캐시 | [excel_data_loader.py](excel_data_loader.py) | 보조 폴백으로만 유지 (API 정상화로 필수 아님) |
...
| **✨ 신규: 인증키** | [config.py](config.py) | data.go.kr 18개 API 공통 SERVICE_KEY 적용 (2026-05-26 발급, 2년 유효) |
| **✨ 신규: smoke test** | [smoke_test_apis.py](smoke_test_apis.py) | 18 API 21 오퍼레이션 자동 검증 — 전부 통과 |
...
### 7-2. 의약외품 (1종) — 마스크 관련 제외
| API | 목적 | 엑셀 NO |
|---|---|---|
| 의약외품 제품 허가정보 | 의약외품 마스터 | 145 |

> ~~마스크 회수(NO 172) / 마스크 행정처분(NO 162)~~ — 프로젝트 범위에서 제외 (2026-05-26 사용자 결정)
...
## 14-3. 인증 체계 (2026-05-26 검증 완료)

### 14-3-1. data.go.kr 계열 SERVICE_KEY (Primary)
- 발급일: 2026-05-26 / 활용기간 2년
- **단일 키로 23개 API 신청 완료** (18 작동 + 5 End Point 확인 대기)
- 보안: 운영 환경에서 반드시 `PUBLIC_DATA_SERVICE_KEY` 환경변수로 주입 ([config.py](config.py) fallback 제거 필요)

### 14-3-2. foodsafetykorea.go.kr 별도 인증 (Fallback)
- **선택적 발급** — data.go.kr NO 339 작동 시 불필요
- data.go.kr NO 339 End Point 확인 후에도 작동하지 않을 경우 폴백 채널로 활용
- URL 패턴: `http://openapi.foodsafetykorea.go.kr/api/{KEY_ID}/{SERVICE_ID}/{TYPE}/{START}/{END}`
- 신청: https://www.foodsafetykorea.go.kr/api/userApiKey.do
- NO 339 서비스 ID = `I0490`, 응답 필드 19개 ([API_STATUS.md](API_STATUS.md) 참조)
- 향후 푸드QR·HACCP 확장 시 활용

### 14-3-3. 코드 분리 원칙
[config.py](config.py) 가 두 체계를 분리 관리:
- `API_ENDPOINTS` dict — data.go.kr Primary 채널
- `FOODSAFETY_SERVICES` dict — 폴백 채널 (선택적)
- 호출은 [api_client/](api_client/) 의 `_call_data_go_kr()` vs `_call_foodsafety()` 로 패키지 분할 예정
...
| v1.1 | 2026-05-26 | API 18개 승인·smoke test 21/22 통과 반영 / NO 140 의약품 허가정보 404 해결(함수명 오타) / 식품안전나라(NO 339) 별도 인증 체계 명시 / [API_STATUS.md](API_STATUS.md) · [smoke_test_apis.py](smoke_test_apis.py) 신규 자료 추가 |
| v1.2 | 2026-05-26 | 신규 5개 승인 반영(NO 339·225·444·470·477) — 동일 SERVICE_KEY / End Point 자동 탐색 실패로 명세 확인 대기 / 마스크 관련 2개(NO 162·172) 프로젝트 제외 결정 / 의약외품 1종으로 축소 / 식품안전나라는 폴백 채널로 격하 |
